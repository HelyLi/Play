...
abstract class CalendarPagerView extends ViewGroup implements View.OnClickListener {
    ...
    private Drawable mDaysBackground;
    ...
    public CalendarPagerView(@NonNull MaterialCalendarView view,
        ...
        mDaysBackground = getDaysBackground();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!dayViews.isEmpty()) {
            int top = ((List<DayView>)dayViews).get(0).getTop();
            canvas.save();
            mDaysBackground.setBounds(getLeft(), top, getRight(), getBottom());
            mDaysBackground.draw(canvas);
            canvas.restore();
        }
    }
