...
import com.github.freeman0211.library.address.R;
	...
	public void setCurrentItem(int index, boolean animated) {
		...
		if (index != currentItem) {
			if (animated) {
				...
				int old = currentItem;
				boolean isAdd = currentItem < index;
				//if scroll up current0--1 current1--0 current2--1
				mLastView = itemsLayout.getChildAt(currentItem < 2 ? currentItem : (isAdd ? 1 : 2));
				if (mLastView != null) {
					if (mLastView instanceof TextView) {
						((TextView) mLastView).setTextColor(Color.parseColor("#b4b4b4"));
					} else {
						mLastView = mLastView.findViewById(R.id.addr_item_tv);
						if (mLastView != null) {
							((TextView) mLastView).setTextColor(Color.parseColor("#b4b4b4"));
						}
					}
				...
				currentItem = index;
				mCurrentView = itemsLayout.getChildAt(currentItem < 2 ? currentItem : (isAdd ? 2 : 1));
				if (mCurrentView != null) {
					if (mCurrentView instanceof TextView) {
						((TextView) mCurrentView).setTextColor(Color.parseColor("#1c1c1c"));
					} else {
						mCurrentView = mCurrentView.findViewById(R.id.addr_item_tv);
						if (mCurrentView != null) {
							((TextView) mCurrentView).setTextColor(Color.parseColor("#1c1c1c"));
						}
					}
		...
		if (view != null) {
			if (view instanceof TextView) {
				if (index != currentItem) {
					...
					((TextView) view).setTextColor(Color.parseColor("#1c1c1c"));
				}
			}
			if (first) {
